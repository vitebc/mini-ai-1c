...
`FormName` проверяется как идентификатор 1С до первой записи (буквы, цифры и подчёркивание; первый символ — не цифра): иначе отказ с exit 1.

## Команда

```powershell
powershell.exe -NoProfile -File 1c-form-add/scripts/form-add.ps1 -ObjectPath "<ObjectPath>" -FormName "<FormName>" [-Purpose "<Purpose>"] [-Synonym "<Synonym>"] [-SetDefault]
```
