...
## Зеркало round-trip (1.8.0)

Декомпилятор восстанавливает конструкции компилятора:

- Строки вне именованных областей → поле `rows` верхнего уровня; области без имени не создаются
- Виды областей по осям: только строки → `Rows`, только колонки → `Columns`, обе оси → `Rectangle` (в `namedAreas[]` при координатных областях)
- Наборы колонок → `columnSets` с именами `nabor1`, `nabor2` и явным `id`; ссылки строк/областей — полем `columnSet`
- Мультиязычные тексты → объекты `{ "ru": "...", ... }`, языки — в `languages`/`currentLanguage`/`defaultLanguage`
- Дробные кегли шрифтов, `Justify`/`Bottom` выравнивания сохраняются
- Плотные строки без пропусков сворачиваются в строки-массивы
- Собственные стили строк → поле `style` (в отличие от `rowStyle` без автозаполнения пустот)
- `-OutputPath` может быть абсолютным путём (каталог создаётся автоматически)
