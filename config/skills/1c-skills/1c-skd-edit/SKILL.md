...
```powershell
powershell.exe -NoProfile -File 1c-skd-edit/scripts/skd-edit.ps1 -TemplatePath "<path>" -Operation <op> -Value "<value>"
```
...
Пустое значение (нет правой части, `_`, `null`) при добавлении не пишется элементом `<value>` — так же читает компилятор схемы. Причина: платформа отвергает схемы, где маркер пустоты просочился значением (`XDTO при чтении Template.xml` на 8.3.27.2214).
...
`value=_` / `value=null` при изменении снимает элемент `<value>` вместе с предшествующим отступом (пустое значение — это ОТСУТСТВИЕ элемента, а не пустой текст).
