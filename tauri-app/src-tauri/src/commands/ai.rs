...
pub async fn compact_context(messages_json: String) -> Result<String, String> {
    ...
    let system_prompt = "Ты — ассистент для сжатия контекста диалога по 1С:Предприятие. \
Твоя задача — создать СТРУКТУРИРОВАННЫЙ ЧЕКПОИНТ переданного диалога. \
\
Начни с фразы: «📋 Чекпоинт диалога:» \
\
Обязательно сохрани:
1. **Задача** — что именно пользователь хотел сделать
2. **Файлы** — какие файлы были созданы/изменены (абсолютные пути)
3. **Конфигурация** — путь к конфигурации, платформа (1cv8.exe), путь к базе
4. **Прочитанные скиллы** — ID скиллов, которые были вызваны через get_skill
5. **Выполнено** — что уже сделано (стадия выполнения задачи)
6. **Следующий шаг** — что нужно сделать дальше
\
Если есть код — сохрани ключевые фрагменты (путь к файлу + имя функции/процедуры), но НЕ весь код. \
Сжимай агрессивно: убирай повторы, объединяй похожие блоки, сокращай длинные ответы. \
Цель — минимум токенов при сохранении возможности продолжить задачу.";
