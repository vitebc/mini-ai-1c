...
export function CommandMenu({ commands, onSelect, onClose, anchorRect }: CommandMenuProps) {
    ...
    return (
        <div
            ref={menuRef}
            className="absolute bottom-full left-0 mb-2 w-full max-w-[580px] bg-zinc-800 border border-zinc-700 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200 ring-1 ring-black/50"
            style={{
                maxHeight: '300px',
                overflowY: 'auto'
            }}
        >
            <div className="flex items-center gap-2 px-3 py-2 bg-zinc-700/50 border-b border-zinc-700 text-[10px] text-zinc-500 uppercase tracking-wider font-bold">
            ...
            {commands.map((cmd, index) => (
                <div
                    ...
                    className={`px-4 py-3 cursor-pointer transition-all flex items-start gap-4 ${index === selectedIndex ? 'bg-blue-600/10 border-l-2 border-blue-500' : 'hover:bg-zinc-700 border-l-2 border-transparent'
                        }`}
                >
